...
# we will make a hard break of using the new data in 2010. 
  ...
  massbalance <- mbind(pre2010, post2010)
   ...
   fb <- mbind(fb, openingStocks)
    ...
    removethem <- c(
      ...
      "2562|Palm kernels",
      "2960|Fish, Seafood"
      #cereals get replaced with their SUA items, the FB items don't account for milling processes
  ...
  # everything else is too specific for our purposes here
    ...
    oilCrops <- oilCrops[grep("Maize (corn)|Cotton|Rape|Mustard|Coco|Sesame|Soya|Groundnut|Kapok|inseed|Sunflower|Hemp|Other oil seeds|Oliv", oilCrops)]
   ...
   sua <- sua[, , intersect(getItems(sua, dim = 3.1), keep)]
    ...
    attributesWMFB <- (prodAttributesFB / dimSums(prodAttributesFB[, , "wm"], dim = "attributes"))
    attributesWMSUA <- (prodAttributesSUA / dimSums(prodAttributesSUA[, , "wm"], dim = "attributes"))
    ...
    .checkAndClear <- function(object,
      ...
      if (extractionBasis == "input") {
       ...
       # move from "from" to "process" and clear "from"
        ...
        if (from != process) {
             object[, , list(goodsIn, process)] <- dimSums(object[, , list(goodsIn, from)], dim = 3.2)  + object[, , list(goodsIn, process)]
        }
    ...
    .extractGoodFromFlow <- function(object,
      ...
      object <- .checkAndClear(object, objectO = NULL, goodIn, from, process, reportAs, residual, attrNoWM)
    ...
    .processingGlobal <- function(object,
      ...
      if (residual != "food" && residual != "alcoholloss" && any(object[, , list(goodsIn, c(reportAs, residual))] != 0)) {
    ...
    if (!is.null(extractionFactor)) {
     ...
     object[, , list(goodsIn, process)] <- (dimSums(ratioIns, dim = "ElementShort") * 
                                                              dimSums(object[, , list(goodsOut, "production")][, , extractionAttribute],
                                                                    dim = c("ElementShort", "ItemCodeItem"))  * extractionConversion) +
    ...
    object[, , list(goodsIn, reportAs[j])] <- ratioIns * dimSums(object[, , list(goodsOut[j], "production")], dim = c("ElementShort", "ItemCodeItem")) +
                                                  object[, , list(goodsIn, reportAs[j])]
  
   object[, , list(goodsOut[j], "production_estimated")] <- dimSums(object[, , list(goodsOut[j], "production")], dim = "ElementShort")
        }

   object[, , list(goodsIn, residual)] <- (dimSums(object[, , list(goodsIn, process)], dim = c("ElementShort", "ItemCodeItem"))
                                            - (dimSums(object[, , list(goodsIn, reportAs)], dim = c("ElementShort", "ItemCodeItem"))))
     ...
     object[, , list(goodsIn, from)] <- (dimSums(object[, , list(goodsIn, from)], dim = c("ElementShort"))
      ...
      # check results and clear processed position
      object <- .checkAndClear(object, objectO, goodsIn, from,
  ...
  # processing of maiz and sugar cane  (other_util) to ethanol, distillers grain and distilling loss
    ...
    "
      ...
      # ethanol processing from tece and maize (ethanol1, distillers_grain, and distillingloss)
        ...
        object[, , c(prodIn, "X002|Distillers_grain")] <- .extractGoodFromFlow(
...
# processing of sugar cane and sugar beet (processed) to sugar1, molasses1 and refiningloss,
    ...
    .sugarProcessing <- function(object) {
      ...
      object[, , c(starches, goodsOut)] <- .processingGlobal(object = object[, , c(starches, goodsOut)],
                                                            goodsIn = goodsIn,
                                                            from = "processed",
                                                            process = "refining",
                                                            goodsOut = goodsOut,
      ...
      for (i in seq_along(starches)) {
        ...
        object[, , list(names(starches)[[i]], "refining")]  =  dimSums(object[, , list(starches[[i]], c("sugar1", "sugar2"))], dim = 3.2)                                                                  
        object[, , list(names(starches)[[i]], c("sugar1", "sugar2"))] = object[, , list(starches[[i]], c("sugar1", "sugar2"))]

   # if process_estimated already exists (i.e. maize from the fermenting step before, add it on)
          object[, , list(names(starches)[[i]], "process_estimated")] =  object[, , list(names(starches)[[i]], "process_estimated")] + 
                                                                        object[, , list(starches[[i]], "process_estimated"), drop = TRUE]
      }
    
   # sometimes there are starches that are imported, or appear out of thin air, or glucose production appears without starch processing
   # here we need to make negatives 0, still tracked in process_estimated 
    ...
    .beerProcessing <- function(object) {  ### do it product specific 
      ...
      object[, , c(beerCereals[j], beersOut[j])] <- .processingGlobal(object = object[, , c(beerCereals[j], beersOut[j])],
        ...
        object[, , c(beerCereals[j], "X004|Brewers_grain")] <- .extractGoodFromFlow(object = object[, , c(beerCereals[j], "X004|Brewers_grain")],
  ...
  .cerealMilling <- function(object) {
      ...
      for (j in seq_along(cropsIn)) {
        ...
        object[, , c(cropsIn[j],  bransOut[j])] <-  .processingGlobal(object = object[, , c(cropsIn[j], bransOut[j])],
    ...
    object[, , c(riceIn, roOut, rcOut)]<-  .processingGlobal(object = object[, , c(riceIn, roOut, rcOut)],
    ...
    object[, , c(maizIn, brOut, moOut, mcOut)]<-  .processingGlobal(object = object[, , c(maizIn, brOut, moOut, mcOut)],
                                                                           objectO = object[, , c(maizIn, brOut, moOut, mcOut)],
                                                                                goodsIn = maizIn,
                                                                                from = "processed",
                                                                                process = "milling",
...
# we combine olives with rapeseed here as they have no cakes, leading to imbalcnce in processing conversion later on when the rapeseed k category
# gets harmonized. Mostly Spain now produces very expensive rapsoil  
  object[, , "270|Rape or colza seed"] <-  object[, , "270|Rape or colza seed"] + dimSums(object[ , , "260|Olives"], dim = 3.1)
  object[, , "271|Rapeseed or canola oil, crude"] <-  object[, , "271|Rapeseed or canola oil, crude"] + dimSums(object[ , , "261|Olive oil"], dim = 3.1)
    ...
    .massbalanceProcessing <- function(years) {
      ...
      millingDimensions <- c("production", "production_estimated", "process_estimated", "intermediate", "extractionloss",
                             "milling", "processed", "brans1",
